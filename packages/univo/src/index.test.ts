...
test("public_writeBlocks returns no results when provided a block that matches no event filters", async () => {
	const univo = indexer({ quiet: true, signingKey: "test", getBlock: test_getBlock });
...
test("public_writeBlocks returns ok results if we return an empty handler", async () => {
	const univo = indexer({ quiet: true, signingKey: "test", getBlock: test_getBlock });
...
test("public_writeBlock returns ok results if we successfully upsert events", async () => {
	const upserted: any[] = [];
...
test("public_writeBlock returns a block_error for all events when failing to load a block", async () => {
	const univo = indexer({
		quiet: true,
		signingKey: "test",
		getBlock: async () => {
...
test("public_writeBlock records a handler_error", async () => {
	const univo = indexer({ quiet: true, signingKey: "test", getBlock: test_getBlock });
...
test("public_writeBlock returns any upsert errors", async () => {
	const univo = indexer({ quiet: true, signingKey: "test", getBlock: test_getBlock });
...
test("public_writeBlock retries upsert errors", async () => {
	let count = 0;
	const univo = indexer({ quiet: true, signingKey: "test", getBlock: test_getBlock });
...
test("public_writeBlock deduplicates events with the same storage adapter", async () => {
	let count = 0;
	let batch = [] as any[];
